---
title: "Migrating FOG Server (1.5)"
description: How to move a FOG 1.5 server's settings, database, images, and certificate trust from an old server to a new one
aliases:
    - "Migrating FOG Server (1.5)"
    - "FOG Server Migration (1.5)"
    - "Moving FOG To A New Server (1.5)"
context_id: "migrating-fog-server-1.5"
tags:
    - install
    - migrating
    - new-server
...
Either way, the old and new FOG servers do not need to be on the exact same
1.5.x point release — you can go from an older 1.5.x to a newer one (or from
`stable` to `dev-branch`), but not the reverse, so don't install an older FOG
version on the new box than the old one is running. If instead you want to
...
To install FOG itself, follow [[1.5/installation/server/install-fog-server|Install FOG Server (1.5)]] — check [[requirements|System Requirements]] first.
Installing FOG here is no different than any other fresh install; nothing
about it changes because you're migrating.
...
FOG split its certificates into per-purpose *zones* under `/opt/fog/pki/` —
and that split has already reached the `dev-branch` line, so a current 1.5.x
server built from `dev-branch` is post-PKI too. It's only an older 1.5.x
install (an older `stable` release, or `dev-branch` from before the zone
split) that's still on the flat, pre-PKI layout. Ask the **old** server which
it is:
...
| | Pre-PKI (older FOG 1.5.x) | Post-PKI (current `dev-branch`, and 1.6) |
|---|---|---|
...
`-C`/`--recreate-CA` (see [[1.5/installation/server/command-line-options|Fog installer command line options (1.5)]]). Every existing client keeps trusting the server with no
...
>Migrating an older pre-PKI 1.5.x install to another server still on the
>pre-PKI layout, the installer tests for the CA **key**, not the certificate.
>There `snapins/ssl` really is the only path to copy — but it has to include
>`CA/.fogCA.key`, or the new server quietly builds a brand new CA and every
>fog-client stops trusting it.
...
>certificate — exactly the repeated work this section exists to avoid.
...
>1.6 also lets you bring your own Secure Boot **intermediate** — the
>certificate enrolled in firmware — separately from the signing leaf. 1.5's
>installer only takes the key/certificate pair above; there's no separate
>intermediate flag. See [[1.5/installation/server/command-line-options#Secure Boot options|Secure Boot options (1.5)]] for the full option set.
...
# from an existing FOG 1.5.x server onto this one, then installs FOG here.
#
...
FOG_BRANCH="stable"   # or "dev-branch" -- match or exceed the old server's branch, never go older
IMAGES_DIR="/images"
...
# Post-PKI servers (current dev-branch, and any 1.6) keep the root CA's
# PRIVATE KEY, the web zone and the Secure Boot zone here instead -- the
# snapins copy above carries only the certificate. Missing this leaves a
# server that reads its own root as offline and cannot issue the Web CA
# beneath it.
...
> [[1.5/installation/server/install-fog-server#Installer Prompts|Install FOG Server (1.5)]]; the `mysqldump`/`mysql` steps assume interactive,
> password-based MySQL auth and will need adjusting if your servers use
> passwordless/socket auth or an external database; and `-B fog` faithfully
> drops and recreates every table FOG installed by default on the new server,
> replacing it with the old server's data — expected here, but worth knowing
> before you run it a second time by accident.
...
-   Update DHCP option 66 to the new server's IP or DNS name — see
    [[1.5/installation/network-setup/dhcp-server-settings|DHCP Server Settings (1.5)]] for the current configuration examples (ISC DHCP, Windows Server).
-   If you don't control the DHCP server, or it can't set options 66/67,
    use [[1.5/installation/network-setup/proxy-dhcp|Proxy DHCP with dnsmasq (1.5)]] instead.
