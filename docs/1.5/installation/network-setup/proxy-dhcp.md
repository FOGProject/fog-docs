---
title: "Proxy DHCP with dnsmasq (1.5)"
description: Configuration of a proxy dhcp service like dnsmasq to use FOG 1.5 as a pxeboot server
context_id: "proxy-dhcp-1.5"
aliases:
    - "Proxy DHCP with dnsmasq (1.5)"
    - "Proxy DHCP using DNSMasq (1.5)"
    - "Using FOG with an unmodifiable DHCP server (1.5)"
    - "Proxy DHCP (1.5)"
tags:
    - pxe
    - ipxe
...
>the file names below — including the signed Secure Boot chain — are the
>same on both versions. What genuinely differs on 1.5 is **how the UEFI boot
>script reaches the client for FOG's own (non-Secure-Boot) builds**, and
>**Secure Boot is unavailable once you turn HTTPS on**. Both are covered
>under [Secure Boot and proxyDHCP](#secure-boot-and-proxydhcp) below.

| Client type | DHCP arch | Boot file |
| --- | --- | --- |
| BIOS / legacy | `00000` | `undionly.kkpxe` |
| 32-bit UEFI | `00006` | `i386-efi/snponly.efi` |
| 64-bit UEFI | `00007`/`00008`/`00009` | `snponly.efi`, or `secureboot/snponly-shimx64.efi` for the signed chain |
| ARM64 UEFI | `00011` | `arm64-efi/snponly.efi`, or `secureboot/arm64-efi/snponly-shimaa64.efi` for the signed chain |

**Use the Secure Boot column.** The examples on this page do, and so does the
FOG 1.5 installer when it configures its own DHCP server. Those files are
safe to hand to *every* 64-bit UEFI and ARM64 client, whether Secure Boot is
on or off — see [Secure Boot and proxyDHCP](#secure-boot-and-proxydhcp) for
why — **as long as this server's web UI is on plain HTTP.** If it's on
HTTPS, see the warning in that section: the signed chain isn't staged at all
on a 1.5 HTTPS install, and you need the plain (unsigned) column instead.
...
# is unconditional rather than a per-machine opt-in, and for why it does NOT
# apply if this server's web UI is on HTTPS.
...
(`undionly.kkpxe` for BIOS, `secureboot/snponly-shimx64.efi` for 64-bit UEFI).
What happens next depends on the binary. The signed Secure Boot chain always
fetches a plain-text `autoexec.ipxe` over TFTP and runs that; FOG's own
**plain, unsigned** UEFI builds (`snponly.efi`, `i386-efi/snponly.efi`,
`arm64-efi/snponly.efi`) have that script **compiled in** on 1.5, and never
read `autoexec.ipxe` at all — see
...
for the full detail. Either way the script chainloads
`tftp://<fog_server_IP>/default.ipxe`, which in turn chainloads your FOG
server's boot script over HTTP or HTTPS.
...
> [!note] FOG 1.5 + HTTPS
> Unlike 1.6, FOG 1.5 has no separate controls for the web UI protocol, the
> netboot protocol, and whether iPXE gets rebuilt — turning on HTTPS
> (`-S`/`--force-https`, or answering yes to the installer's HTTPS prompt)
> switches all three at once, including a 10–25 minute local iPXE rebuild.
> See [[1.5/installation/server/command-line-options#HTTPS and the -S flag|HTTPS and the -S flag (1.5)]].
>
...
> certificate error after iPXE loads. And point DHCP at FOG's own unsigned
> builds rather than `secureboot/` — see the warning below.
...
So the correct configuration, **as long as this server's web UI is on plain
HTTP**, is not a conditional — it is to point **every** 64-bit UEFI and ARM64
client at the Secure Boot files unconditionally. That set is a superset: it
covers the Secure Boot machines and costs the others nothing.

>[!warning] HTTPS and the signed chain don't mix on 1.5
>1.5 has no way to switch the web UI to HTTPS without also rebuilding iPXE
>locally with this server's own CA embedded, and a locally rebuilt binary
>can't carry Microsoft's signature — so on an HTTPS 1.5 install, **the
>`secureboot/` binaries are never staged at all**, and DHCP has nothing to
...
[Dns Masq LTSP settings](#dns-masq-ltsp-settings). That block points
every 64-bit UEFI and ARM64 client at the signed chain and leaves BIOS and
32-bit UEFI on their unsigned binaries, which is the whole of it (assuming
this server is on plain HTTP — see the warning above if it isn't). There is
no separate "Secure Boot config" to switch to.
...
nothing signed to point an ia32 UEFI client at, on either FOG version. Such
machines must have Secure Boot **disabled** to network boot.
