...
description: Fixing a missing or pending primary MAC address on a FOG host, via the API or directly in the database
...
>[[1.5/kb/troubleshooting/primary-mac-address-issues|1.5 version]] of this
>page for FOG 1.5 — the API and database fix steps below work the same way on
>that version, using the token-pair or Basic auth methods rather than a
>Bearer token.
...
The primary mac for a host is set in the `hostMAC` table. Each entry in that table has a `hmHostID` the mac is linked to and a boolean `hmPrimary` field (1 or 0).
...
If you don't want to use the API for this, you can edit the database directly.
...
In the database you would update the row in the `hostMAC` table to have `hmPending="0"` where `hmPrimary="1"` for that `hmHostID`
