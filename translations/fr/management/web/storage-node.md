...
- Définition : les nœuds de stockage fournissent un [espace de stockage NFS/FTP supplémentaire](http://www.fogproject.org/wiki/index.php?title=InstallationModes) qui augmente le débit disponible et la redondance au sein d'un réseau. Ils ne fournissent pas de services PXE, TFTP ou DHCP sur les sites secondaires. Pour activer des services PXE et TFTP supplémentaires sur des sites secondaires, voir cette section : [#Including multiple PXE / TFTP servers](#Including_multiple_PXE_.2F_TFTP_servers "wikilink")
